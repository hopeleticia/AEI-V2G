Import-Module Posh-SSH
$piPassword = $env:AEI_PI_PASSWORD
if (-not $piPassword) { throw "Set AEI_PI_PASSWORD before running this script." }
$pw = ConvertTo-SecureString $piPassword -AsPlainText -Force
$nodes = @(
  @{ id="pi1"; user="pi1"; ip="192.168.137.217" },
  @{ id="pi2"; user="pi2"; ip="192.168.137.246" },
  @{ id="pi3"; user="pi3"; ip="192.168.137.13"  },
  @{ id="pi5"; user="pi5"; ip="192.168.137.16"  },
  @{ id="pi6"; user="pi6"; ip="192.168.137.104" }
)
foreach ($node in $nodes) {
  Write-Host "=== $($node.id) ==="
  $cred = New-Object System.Management.Automation.PSCredential($node.user, $pw)
  try {
    $s = New-SSHSession -ComputerName $node.ip -Credential $cred -AcceptKey -Force -ConnectionTimeout 20 -ErrorAction Stop
    $r = Invoke-SSHCommand -SessionId $s.SessionId -TimeOut 300 -Command @'
# 1. paho-mqtt via apt -> /usr/lib/python3/dist-packages (already in sys.path, survives reboots)
sudo apt-get install -y python3-paho-mqtt 2>&1 | tail -2

# 2. web3 via pip --user -> ~/.local/lib/python3.13/site-packages (PYTHONPATH set at launch)
pip3 install --user 'web3>=6.0.0' 2>&1 | tail -2

# Verify paho is importable without any PYTHONPATH tricks
python3 -c "import paho.mqtt.client; print('paho_ok')" || echo PAHO_FAIL

# Verify web3 importable with PYTHONPATH (simulates launch environment)
PYTHONPATH=/home/$USER/.local/lib/python3.13/site-packages python3 -c "import web3; print('web3_ok')" || echo WEB3_FAIL

echo DONE
'@
    Write-Host "$($node.id): $($r.Output)"
    Remove-SSHSession -SessionId $s.SessionId | Out-Null
  } catch { Write-Host "$($node.id): OFFLINE - $($_.Exception.Message)" }
}
Write-Host "All done."
