...
foreach ($node in $nodes) {
  ...
  $r = Invoke-SSHCommand -SessionId $s.SessionId -Command "echo '=== $($node.id) ==='; pgrep -a python3; cat /tmp/aei_$($node.id).log 2>/dev/null; ls /tmp/aei_$($node.id).log 2>/dev/null && wc -l /tmp/aei_$($node.id).log; echo END"
  Write-Host $r.Output
